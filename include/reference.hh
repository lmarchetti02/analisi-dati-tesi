...
namespace reference_algorithm
{
     ...
     */
    int max_energy_index(std::vector<Double_t> energies)
    {
     ...
     */
    std::set<Int_t> get_surrounding_ids(Int_t pixel_0, int n_pixel)
    {
        ...
        if (ID_y > 0)
            surrounding_ids.insert(pixel_0 - n_pixel); // T
        if (ID_x > 0)
            surrounding_ids.insert(pixel_0 - 1); // T
        if (ID_x < n_pixel - 1)
            surrounding_ids.insert(pixel_0 + 1); // T
        if (ID_y < n_pixel - 1)
            surrounding_ids.insert(pixel_0 + n_pixel); // T
        if (ID_y > 0 && ID_x > 0)
            surrounding_ids.insert(pixel_0 - n_pixel - 1); // TR
        if (ID_y > 0 && ID_x < n_pixel - 1)
            surrounding_ids.insert(pixel_0 - n_pixel + 1); // TR
        if (ID_y < n_pixel - 1 && ID_x > 0)
            surrounding_ids.insert(pixel_0 + n_pixel - 1); // TR
        if (ID_y < n_pixel - 1 && ID_x < n_pixel - 1)
            surrounding_ids.insert(pixel_0 + n_pixel + 1); // TR
    ...
    event_info cluster(event_info data, int n_pixel)
    {
        ...
        std::set_intersection(surrounding_ids.begin(), surrounding_ids.end(),
                              ids.begin(), ids.end(),
